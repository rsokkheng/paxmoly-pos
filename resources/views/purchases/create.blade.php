    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:16px;align-items:end;">
        ...
        <div class="form-group" style="margin-bottom:0;">
            ...
            <select name="status" class="form-control" required>
                ...
                <option value="received" {{ old('status') === 'received' ? 'selected' : '' }}>Received (adds stock now)</option>
            </select>
        </div>
        <div class="form-group" style="margin-bottom:0;">
...
<div id="payFooter" style="position:fixed;bottom:0;left:0;right:0;z-index:100;background:var(--surface);border-top:2px solid var(--border);padding:12px 24px;display:flex;align-items:center;gap:24px;flex-wrap:wrap;">
    ...
    <div style="display:flex;flex-direction:column;gap:4px;">
        ...
        <input type="number" id="paidAmount" name="_paidProxy" step="0.01" min="0" value="{{ old('paid_amount', 0) }}"
        ...
        <input type="hidden" name="paid_amount" id="paidAmountHidden" form="purchaseForm" value="{{ old('paid_amount', 0) }}">
    </div>
    <div style="display:flex;flex-direction:column;">
...
function applyProduct(idx, product, unit, packSizeOverride) {
    ...
    document.getElementById('unitDesc_' + idx).innerHTML = resolvedUnit === 'carton' && product.has_case
...
function updateSummary() {
    ...
    document.querySelectorAll('#itemsBody tr.item-row').forEach(tr => {
        ...
        totalItems++;
        grand += qty * cost;
        if (unit === 'carton') { totalCases += qty; totalPcs += qty * packSize; }
        else { totalPcs += qty; }
    });
