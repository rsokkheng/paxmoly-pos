@extends('layouts.app')
@section('title', 'Create Product Set')
@section('page-title', 'Create Product Set')
@section('breadcrumb', 'Products / Sets / Create')

@section('content')
<form method="POST" action="{{ route('product-sets.store') }}" enctype="multipart/form-data">
@csrf
...
<div style="display:grid;grid-template-columns:1fr 340px;gap:16px;align-items:start;">
        ...
        <div class="card">
            ...
            <div style="padding:20px;display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <div class="form-group" style="grid-column:1/-1;">
                    ...
                    <input type="text" name="name" class="form-control" value="{{ old('name') }}" required placeholder="e.g. Weekend Combo">
                    @error('name')<div style="color:var(--danger);font-size:11px;margin-top:4px;">{{ $message }}</div>@enderror
                </div>
                <div class="form-group">
                    <label class="form-label">Set Code / SKU <span style="color:var(--danger)">*</span></label>
                    <input type="text" name="code" class="form-control" value="{{ old('code') }}" required placeholder="SET-001" style="text-transform:uppercase;">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="selling_price" class="form-control" value="{{ old('selling_price', 0) }}" min="0" step="0.01" required>
    ...
    <div style="display:flex;flex-direction:column;gap:16px;">
        ...
        <div class="card">
            ...
            <div style="padding:16px;">
                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', '1') ? 'checked' : '' }} style="width:16px;height:16px;">
        ...
        <div style="display:flex;gap:8px;">
            <button type="submit" class="btn btn-primary" style="flex:1;"><i class="fas fa-save"></i> Save Set</button>
...
function addItem(productId, unitType, qty) {
    ...
    row.innerHTML = `
        <div>
            ...
            <select name="items[${idx}][product_id]" class="form-control item-product" onchange="onProductChange(this,${idx})" required>
    ...
    // Pre-select product / unit if provided
    if (productId) {
